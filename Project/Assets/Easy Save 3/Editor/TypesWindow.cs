...
namespace ES3Editor
{
	public class TypesWindow : SubWindow
	{
		...
		int selectedType = -1;
		private ES3Reflection.ES3ReflectedField[] fields = new ES3Reflection.ES3ReflectedField[0];
		...
		private void DisplayFieldsOrProperties(bool showFields, bool showProperties)
		{
			...
			if(fieldCount > 0 || propertyCount > 0)
			{
				...
				//GUILayout.Label("Select:", style.boldLabelNoStretch);
			...
			for(int i=0; i<fields.Length; i++)
			{
				...
				if(typeof(UnityEngine.Object).IsAssignableFrom(field.FieldType))
					content.tooltip = field.FieldType.ToString() + "\nSaved by reference";
				else
					content.tooltip = field.FieldType.ToString() + "\nSaved by value";
		...
		private string GenerateWrites()
		{
			...
			for(int i=0; i<fields.Length; i++)
			{
				...
				if(!field.IsPublic)
					writes += String.Format("\n\t\t\twriter.WritePrivateProperty{1}(\"{0}\", instance);", field.Name, writeByRef, es3TypeParam);
				else
		...
		private string GenerateReads()
		{
			...
			for(int i=0; i<fields.Length; i++)
			{
				var field = fields[i];
				var selected = fieldSelected[i];
				var es3Type = ES3TypeMgr.GetOrCreateES3Type(field.FieldType);

				if(!selected || isComponent && (field.Name == "tag" || field.Name == "name"))
					continue;

				string fieldTypeName = GetFullTypeName(field.FieldType);
				string es3TypeParam = HasExplicitES3Type(es3Type.type) ? es3Type.GetType().Name+".Instance" : "";
		...
		{
			return ES3EditorUtility.PathToEasySaveFolder() + "Types/ES3Type_"+type.Name+".cs";
		}
